...
DEFAULT_OUTPUT_DIR = "outputs/smolvlm_lora"
DEFAULT_ARTIFACTS_ROOT = "submission+artifacts/runs"
...
CHECKPOINT_CONFIG_DEFAULTS = {
    ...
    "include_grade_subject": False,
    "answer_format": "index",
    "score_normalization": "auto",
    "train_objective": "generation",
    "choice_score_normalization": "auto",
}
...
def eval_split_name_from_path(path: Path) -> str:
    parent = path.parent.name
    if parent and parent not in {"data", "."}:
        return sanitize_run_name(f"{parent}_{path.stem}")
...
def resolve_eval_csv_path(args: argparse.Namespace, split: Optional[str] = None) -> Tuple[Path, str]:
    ...
    if eval_csv:
        path = Path(eval_csv)
        return path, eval_split_name_from_path(path)
...
class PromptBuilder:
    ...
    include_grade_subject: bool = False
    ...
    def build_user_text(self, item: Dict[str, Any]) -> str:
        ...
        grade = clean_text(item.get("grade", "")) if self.include_grade_subject else ""
        subject = clean_text(item.get("subject", "")) if self.include_grade_subject else ""
        if grade and subject:
            intro = f"You are solving a {grade} {subject} multiple-choice question."
        elif grade:
            intro = f"You are solving a {grade} science multiple-choice question."
        elif subject:
            intro = f"You are solving a {subject} multiple-choice question."
        else:
            intro = "You are solving a science multiple-choice question."
...
class VlmDataCollator:
    ...
    processor_kwargs: Dict[str, Any]
    answer_format: str
    train_objective: str = "generation"

    def __call__(self, features: Sequence[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        if self.train_objective == "generation":
            return self.collate_generation(features)
        if self.train_objective == "choice_ranking":
            return self.collate_choice_ranking(features)
        raise ValueError(f"Unsupported train_objective: {self.train_objective}")

    def build_labeled_inputs(
        self,
    ...
    def collate_generation(self, features: Sequence[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        return self.build_labeled_inputs(full_texts, prompt_texts, images)

    def collate_choice_ranking(self, features: Sequence[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        for feature in features:
            image = load_rgb_image(feature["image_path"], feature.get("image_bytes"))
            user_text = self.prompt_builder.build_user_text(feature)
            ...
            num_choices = int(feature["num_choices"])
            answer = int(feature["answer"])
            if answer < 0 or answer >= num_choices:
                raise ValueError(f"{feature.get('id')} has invalid answer={answer} for {num_choices} choices.")

            choice_counts.append(num_choices)
            choice_labels.append(answer)
            for candidate in range(num_choices):
                images.append(image)
                ...
                full_texts.append(
                    apply_chat_template(
                        ...
                        build_messages(
                            user_text,
                            answer=format_answer_target(feature, candidate, self.answer_format),
                        ),
                        add_generation_prompt=False,
                    )
                )

        inputs = self.build_labeled_inputs(full_texts, prompt_texts, images)
        inputs["choice_counts"] = torch.tensor(choice_counts, dtype=torch.long)
        inputs["choice_labels"] = torch.tensor(choice_labels, dtype=torch.long)
        return inputs
...
class BalancedTrainer(Trainer):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.choice_score_normalization = choice_score_normalization
    ...
    def compute_loss(
        ...
        if "choice_labels" not in inputs:
            try:
        ...
        choice_counts = inputs.pop("choice_counts")
        labels = inputs.pop("labels")
        outputs = model(**inputs)
        scores = self.score_candidate_sequences(outputs.logits, labels)

        losses = []
        offset = 0
        for label, count in zip(choice_labels.tolist(), choice_counts.tolist()):
            count = int(count)
            if count <= 0:
                raise ValueError("choice_ranking batches require at least one candidate per item.")
            item_scores = scores[offset : offset + count]
            target = torch.tensor([int(label)], dtype=torch.long, device=item_scores.device)
            losses.append(torch.nn.functional.cross_entropy(item_scores.unsqueeze(0), target))
            offset += count

        if offset != int(scores.shape[0]):
            raise ValueError(
                f"Choice-ranking batch had {offset} grouped candidates but {int(scores.shape[0])} scored rows."
            )
        loss = torch.stack(losses).mean()
        return (loss, outputs) if return_outputs else loss

    def score_candidate_sequences(self, logits: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        ...
        if self.choice_score_normalization == "mean":
            token_counts = mask.sum(dim=1).clamp(min=1)
            scores = scores / token_counts
        elif self.choice_score_normalization != "sum":
            raise ValueError(f"Unsupported choice_score_normalization: {self.choice_score_normalization}")
        return scores
...
def add_lora_adapters(model: Any, args: argparse.Namespace) -> Any:
    ...
    use_dora = getattr(args, "lora_use_dora", False)
    use_rslora = getattr(args, "lora_use_rslora", False)
    ...
    print(f"LoRA target modules: {len(target_modules)}{extras_str}")
...
def make_prompt_builder(args: argparse.Namespace) -> PromptBuilder:
    return PromptBuilder(
        ...
        include_grade_subject=getattr(args, "include_grade_subject", False),
...
def metric_value_to_text(value: Any) -> str:
...
class MetricsLogger:
    ...
    def __init__(self, csv_path: Optional[Path]) -> None:
        ...
        self.jsonl_path = csv_path.with_suffix(".jsonl") if csv_path is not None else None
    ...
    def log_event(
        ...
        if self.csv_path is None or self.jsonl_path is None:
            return

        cleaned_metrics = {str(key): json_safe(value) for key, value in metrics.items()}
        cleaned_details = json_safe(details or {})
        ...
        details_text = json.dumps(cleaned_details, sort_keys=True)
                    ...
                    writer.writerow(
                        {
                            ...
                            "metric": metric,
                            "value": metric_value_to_text(value),
...
class TrainingMetricsCallback(TrainerCallback):
    ...
    def on_log(self, args: TrainingArguments, state: Any, control: Any, logs: Optional[Dict[str, Any]] = None, **kwargs: Any) -> None:
        if not logs:
            return
        metrics = dict(logs)
        epoch = metrics.pop("epoch", state.epoch)
...
def resolve_metrics_log_path(args: argparse.Namespace, default_dir: Optional[Path]) -> Optional[Path]:
    metrics_log_path = getattr(args, "metrics_log_path", None)
    if metrics_log_path:
        return Path(metrics_log_path)
    if default_dir is None:
        return None
    return default_dir / "metrics_log.csv"


def make_metrics_logger(args: argparse.Namespace, default_dir: Optional[Path]) -> MetricsLogger:
    return MetricsLogger(resolve_metrics_log_path(args, default_dir))


def accuracy_metrics(accuracy: float, rows: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
...
def default_artifact_run_name(args: argparse.Namespace, command_name: str, default_name: str = "run") -> str:
    run_name = getattr(args, "run_name", None)
    if run_name:
        return sanitize_run_name(run_name)

    if command_name in {"eval", "predict"} and getattr(args, "checkpoint", None):
        checkpoint = Path(args.checkpoint)
        if checkpoint.name == "final_adapter" and checkpoint.parent.name:
            return sanitize_run_name(checkpoint.parent.name)
        return sanitize_run_name(checkpoint.name)

    return sanitize_run_name(default_name)
...
def resolve_artifact_dir(
    ...
    if artifact_dir:
        ...
        run_name = default_artifact_run_name(args, command_name, default_name=default_name)
        resolved = artifacts_root / run_name
    setattr(args, "artifact_dir", str(resolved))
    return resolved
...
def next_available_path(path: Path) -> Path:
    ...
    for index in range(2, 10_000):
        ...
        if not candidate.exists():
            return candidate
    raise RuntimeError(f"Could not find an available path based on {path}")
...
def init_run_artifacts(
    ...
    copy_if_exists(Path(__file__).resolve(), code_dir / Path(__file__).name)
    copy_if_exists(Path("requirements.txt"), config_dir / "requirements.txt")
    copy_if_exists(Path("requirements-cu130.txt"), config_dir / "requirements-cu130.txt")
    payload = {
    ...
    if extra:
        payload["extra"] = extra
    write_json(next_available_path(config_dir / f"{command_name}_args.json"), payload)
    return artifact_dir


def snapshot_training_artifacts(output_dir: Path, artifact_dir: Path) -> None:
    summary_dir = artifact_dir / "training"
    copy_if_exists(output_dir / "best_checkpoint.txt", summary_dir / "best_checkpoint.txt")
    copy_if_exists(output_dir / "final_adapter" / COMPETITION_CONFIG_FILE, summary_dir / COMPETITION_CONFIG_FILE)
    copy_if_exists(output_dir / "final_adapter" / "adapter_config.json", summary_dir / "adapter_config.json")
    copy_if_exists(output_dir / "final_adapter" / "trainer_state.json", summary_dir / "trainer_state.json")
    write_json(
...
def train(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "run_name", None) and args.output_dir == DEFAULT_OUTPUT_DIR:
        args.output_dir = str(Path("outputs") / sanitize_run_name(args.run_name))
    output_dir = Path(args.output_dir)
    artifact_dir = resolve_artifact_dir(args, "train", default_name=output_dir.name)
    if not getattr(args, "metrics_log_path", None):
        args.metrics_log_path = str(artifact_dir / "metrics" / "metrics_log.csv")
    init_run_artifacts(
        args,
        "train",
        artifact_dir,
        extra={
            "output_dir": str(output_dir),
            "recommended_submission_path": str(artifact_dir / "submissions" / DEFAULT_SUBMISSION_FILENAME),
        },
    )
    ...
    choice_score_normalization = resolve_score_normalization(args.answer_format, args.choice_score_normalization)
    if args.train_objective == "choice_ranking":
        print(f"Using choice-ranking training objective with {choice_score_normalization} candidate score normalization.")

    metrics_logger = make_metrics_logger(args, default_dir=output_dir)
    if metrics_logger.csv_path is not None:
        print(f"Writing metrics log to {metrics_logger.csv_path}")
        print(f"Writing metrics JSONL to {metrics_logger.jsonl_path}")
    metrics_logger.log_event(
        "run_start",
        {
            ...
            "train_objective": args.train_objective,
            "choice_score_normalization": choice_score_normalization,
        },
        split="train",
        details={
            "train_csvs": [str(path) for path in train_csvs],
            "eval_csv": args.eval_csv or str(data_dir / "val.csv"),
            "output_dir": str(output_dir),
            "artifact_dir": str(artifact_dir),
        },
    )
    ...
    trainer = BalancedTrainer(
        ...
        train_objective=args.train_objective,
        choice_score_normalization=choice_score_normalization,
        callbacks=[TrainingMetricsCallback(metrics_logger)],
    )
    ...
    metrics_logger.log_event(
        ...
        step=final_step,
        epoch=final_epoch,
        details={"train_samples": len(train_dataset)},
    )
    ...
    needs_reload = args.select_best_checkpoint or not args.skip_eval or getattr(args, "predict_after_train", False)
    if needs_reload:
        del trainer
        del model
        if torch.cuda.is_available():
    ...
    if args.save_final and args.select_best_checkpoint:
        ...
        for candidate_dir in candidate_dirs:
            accuracy, rows, split_name = evaluate_checkpoint(args, checkpoint=str(candidate_dir), split="val")
            print(f"{split_name} accuracy for {candidate_dir.name}: {accuracy:.4f}")
            metrics_logger.log_event(
                "checkpoint_validation",
                accuracy_metrics(accuracy, rows),
                split=split_name,
                details={"checkpoint": str(candidate_dir)},
            )
        ...
        metrics_logger.log_event(
            ...
            split=best_split_name,
            details={"checkpoint": str(best_dir)},
        )
    ...
    if not args.skip_eval:
        ...
        eval_args.predictions_path = str(artifact_dir / "eval" / f"{eval_split_name}_predictions.csv")
        eval_args.metrics_log_path = args.metrics_log_path
        evaluate(eval_args)

        extra_eval_csvs = parse_path_list(getattr(args, "extra_eval_csvs", None))
        for extra_eval_csv in extra_eval_csvs:
            extra_split_name = eval_split_name_from_path(extra_eval_csv)
            extra_args = argparse.Namespace(**vars(args))
            extra_args.checkpoint = str(final_dir)
            extra_args.eval_csv = str(extra_eval_csv)
            extra_args.split = extra_split_name
            extra_args.predictions_path = str(artifact_dir / "eval" / f"{extra_split_name}_predictions.csv")
            extra_args.metrics_log_path = args.metrics_log_path
            evaluate(extra_args)

    if getattr(args, "predict_after_train", False):
        if not args.save_final:
            raise ValueError("Prediction after training requires --save-final so the adapter can be reloaded.")
        predict_args = argparse.Namespace(**vars(args))
        predict_args.checkpoint = str(final_dir)
        predict_args.submission_path = str(artifact_dir / "submissions" / DEFAULT_SUBMISSION_FILENAME)
        predict_args.score_details_path = str(artifact_dir / "score_details" / "test_scores.csv")
        predict(predict_args)

    snapshot_training_artifacts(output_dir, artifact_dir)
...
def score_item(
    ...
    log_probs = torch.log_softmax(shift_logits / temp, dim=-1)
    safe_labels = shift_labels.masked_fill(~mask, 0)
    token_scores = log_probs.gather(-1, safe_labels.unsqueeze(-1)).squeeze(-1)
    scores = (token_scores * mask).sum(dim=1)
...
def evaluate_checkpoint(
    ...
    for item in tqdm(items, desc=f"Scoring {split_name} ({Path(checkpoint).name if checkpoint else 'base'})"):
        scores = score_item(
            ...
            score_temperature=getattr(eval_args, "score_temperature", 1.0),
        )
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "checkpoint", None):
        checkpoint = Path(args.checkpoint)
        default_name = checkpoint.parent.name if checkpoint.name == "final_adapter" else checkpoint.name
    artifact_dir = resolve_artifact_dir(args, "eval", default_name=default_name)
    if not getattr(args, "metrics_log_path", None):
        args.metrics_log_path = str(artifact_dir / "metrics" / "metrics_log.csv")
    ...
    if not getattr(args, "predictions_path", None):
        args.predictions_path = str(artifact_dir / "eval" / f"{split_name}_predictions.csv")
    init_run_artifacts(
        args,
        "eval",
        artifact_dir,
        extra={"checkpoint": str(args.checkpoint), "split_name": split_name},
    )
    metrics_logger = make_metrics_logger(args, default_dir=artifact_dir / "metrics")
    predictions_path_text = None
    if args.predictions_path:
        output_path = Path(args.predictions_path)
        output_path.parent.mkdir(parents=True, exist_ok=True)
        pd.DataFrame(rows).to_csv(output_path, index=False)
        predictions_path_text = str(output_path)
        print(f"Wrote predictions to {output_path}")
    event = "validation" if "val" in split_name.lower() else "evaluation"
    metrics_logger.log_event(
        event,
        accuracy_metrics(accuracy, rows),
        split=split_name,
        details={
            ...
            "predictions_path": predictions_path_text,
        },
    )
    if metrics_logger.csv_path is not None:
        print(f"Updated metrics log at {metrics_logger.csv_path}")
...
def predict(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "checkpoint", None):
        checkpoint = Path(args.checkpoint)
        default_name = checkpoint.parent.name if checkpoint.name == "final_adapter" else checkpoint.name
    artifact_dir = resolve_artifact_dir(args, "predict", default_name=default_name)
    if not getattr(args, "submission_path", None):
        args.submission_path = str(artifact_dir / "submissions" / DEFAULT_SUBMISSION_FILENAME)
    if getattr(args, "score_details_path", None) is None:
        args.score_details_path = str(artifact_dir / "score_details" / "test_scores.csv")
    init_run_artifacts(
        args,
        "predict",
        artifact_dir,
        extra={
            ...
            "score_details_path": str(args.score_details_path),
        },
    )
    ...
    for item in tqdm(items, desc="Scoring test"):
        scores = score_item(
            ...
            score_temperature=getattr(args, "score_temperature", 1.0),
        )
        prediction, adjusted_scores = predict_from_scores(scores, item, score_bias)
    ...
    write_json(
        artifact_dir / "manifest.json",
        {
            ...
            "score_details_path": str(getattr(args, "score_details_path", "")),
            "rows": len(rows),
            "updated_utc": utc_timestamp(),
        },
    )
...
def parse_weight_values(value: Optional[str], count: int) -> List[float]:
    if not value:
        return [1.0] * count
    weights = [float(part.strip()) for part in value.split(",") if part.strip()]
    if len(weights) != count:
        raise ValueError(f"Expected {count} weights, got {len(weights)} from {value!r}.")
...
def load_submission_rows(path: Path) -> List[Dict[str, Any]]:
    ...
    if list(frame.columns) != expected_columns:
        raise ValueError(f"{path} columns must be exactly {expected_columns}, got {list(frame.columns)}")
    rows = []
    for _, row in frame.iterrows():
        answer = row["answer"]
        if not isinstance(answer, (int, np.integer)) and not str(answer).isdigit():
            raise ValueError(f"{path} has non-integer answer {answer!r} for id={row['id']!r}")
        rows.append({"id": clean_text(row["id"]), "answer": int(answer)})
    if not rows:
        raise ValueError(f"{path} did not contain any submission rows.")
    return rows


def validate_submission_rows_against_test(rows: Sequence[Dict[str, Any]], data_dir: Path) -> None:
    test_csv = data_dir / "test.csv"
    if not test_csv.exists():
        print(f"WARNING: skipped validation because {test_csv} does not exist.")
        return

    test_frame = pd.read_csv(test_csv)
    expected_ids = [clean_text(row_id) for row_id in test_frame["id"].tolist()]
    actual_ids = [clean_text(row["id"]) for row in rows]
    if actual_ids != expected_ids:
        raise ValueError("Submission ids or order do not match data/test.csv")

    for row, (_, test_row) in zip(rows, test_frame.iterrows()):
        num_choices = int(test_row.get("num_choices", len(parse_choices(test_row.get("choices", "[]")))))
        answer = int(row["answer"])
        if answer < 0 or answer >= num_choices:
            raise ValueError(
                f"{row['id']} answer {answer} is outside valid range 0..{num_choices - 1}"
            )


def write_submission_rows(rows: Sequence[Dict[str, Any]], output_path: Path, data_dir: Path) -> None:
    validate_submission_rows_against_test(rows, data_dir)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(rows, columns=["id", "answer"]).to_csv(output_path, index=False)
    print(f"Wrote ensemble submission to {output_path}")


def ensemble_vote(args: argparse.Namespace) -> None:
    paths = [Path(path) for path in args.submission_paths]
    if len(paths) < 2:
        raise ValueError("ensemble-vote expects at least two submission CSVs.")
    weights = parse_weight_values(args.weights, len(paths))
    submissions = [load_submission_rows(path) for path in paths]
    ids = [row["id"] for row in submissions[0]]

    for path, rows in zip(paths[1:], submissions[1:]):
        other_ids = [row["id"] for row in rows]
        if other_ids != ids:
            raise ValueError(f"{path} ids/order do not match {paths[0]}")
    ...
    for row_index, row_id in enumerate(ids):
        ...
        answers = []
        for submission_rows, weight in zip(submissions, weights):
            answer = int(submission_rows[row_index]["answer"])
            answers.append(answer)
            weighted_votes[answer] += weight

        best_weight = max(weighted_votes.values())
        tied_answers = {answer for answer, weight in weighted_votes.items() if weight == best_weight}
        ...
        changed_from_first += int(prediction != answers[0])
        unanimous += int(len(set(answers)) == 1)
        output_rows.append({"id": row_id, "answer": prediction})

    write_submission_rows(output_rows, Path(args.output_path), Path(args.data_dir))
    print(f"Inputs: {len(paths)}; rows: {len(output_rows)}; unanimous rows: {unanimous}")
    print(f"Rows changed from first submission: {changed_from_first}")


def select_score_column(frame: pd.DataFrame, requested_column: str, path: Path) -> str:
    if requested_column == "auto":
        if "adjusted_scores" in frame.columns:
            return "adjusted_scores"
        if "scores" in frame.columns:
            return "scores"
        raise ValueError(f"{path} must contain scores or adjusted_scores.")
    if requested_column not in frame.columns:
        raise ValueError(f"{path} does not contain requested score column {requested_column!r}.")
    return requested_column


def load_score_detail_rows(path: Path, requested_column: str) -> List[Dict[str, Any]]:
    frame = pd.read_csv(path)
    required_columns = {"id", "num_choices"}
    missing_columns = required_columns - set(frame.columns)
    if missing_columns:
        raise ValueError(f"{path} is missing required columns: {sorted(missing_columns)}")
    score_column = select_score_column(frame, requested_column, path)

    rows = []
    for _, row in frame.iterrows():
        scores = [float(score) for score in json.loads(clean_text(row[score_column]))]
        num_choices = int(row["num_choices"])
        if len(scores) != num_choices:
            raise ValueError(
                f"{path} id={row['id']!r} has {len(scores)} scores but num_choices={num_choices}"
            )
        rows.append(
            {
                "id": clean_text(row["id"]),
                "num_choices": num_choices,
                "scores": scores,
                "prediction": int(row["prediction"]) if "prediction" in frame.columns else int(np.argmax(scores)),
            }
        )
    if not rows:
        raise ValueError(f"{path} did not contain any score-detail rows.")
    return rows


def normalize_score_vector(scores: Sequence[float], mode: str) -> np.ndarray:
...
def ensemble_scores(args: argparse.Namespace) -> None:
    default_name = Path(args.output_path).stem if getattr(args, "output_path", None) else "ensemble_scores"
    artifact_dir = resolve_artifact_dir(args, "ensemble-scores", default_name=default_name)
    if not getattr(args, "output_path", None):
        args.output_path = str(artifact_dir / "submissions" / DEFAULT_SUBMISSION_FILENAME)
    init_run_artifacts(
        args,
        "ensemble-scores",
        artifact_dir,
        extra={"score_detail_paths": list(args.score_detail_paths), "output_path": str(args.output_path)},
    )
    paths = [Path(path) for path in args.score_detail_paths]
    if len(paths) < 2:
        raise ValueError("ensemble-scores expects at least two score-detail CSVs.")
    weights = parse_weight_values(args.weights, len(paths))
    score_runs = [load_score_detail_rows(path, args.score_column) for path in paths]
    ids = [row["id"] for row in score_runs[0]]
    num_choices = [int(row["num_choices"]) for row in score_runs[0]]

    for path, rows in zip(paths[1:], score_runs[1:]):
        other_ids = [row["id"] for row in rows]
        other_num_choices = [int(row["num_choices"]) for row in rows]
        if other_ids != ids:
            raise ValueError(f"{path} ids/order do not match {paths[0]}")
        if other_num_choices != num_choices:
            raise ValueError(f"{path} num_choices values do not match {paths[0]}")

    output_rows = []
    changed_from_first = 0
    for row_index, row_id in enumerate(ids):
        combined = np.zeros(num_choices[row_index], dtype=np.float64)
        for rows, weight in zip(score_runs, weights):
            combined += weight * normalize_score_vector(rows[row_index]["scores"], args.score_normalize)

        prediction = int(np.argmax(combined))
        changed_from_first += int(prediction != int(score_runs[0][row_index]["prediction"]))
        output_rows.append({"id": row_id, "answer": prediction})

    write_submission_rows(output_rows, Path(args.output_path), Path(args.data_dir))
    print(f"Inputs: {len(paths)}; rows: {len(output_rows)}")
    print(f"Rows changed from first score file prediction: {changed_from_first}")
    write_json(
        artifact_dir / "manifest.json",
        {
            "artifact_dir": str(artifact_dir),
            "output_path": str(args.output_path),
            "score_detail_paths": [str(path) for path in paths],
            "weights": weights,
...
def build_train_command_for_autotune(
    ...
    command = [
        ...
        "--train-objective",
        getattr(args, "train_objective", "generation"),
        "--choice-score-normalization",
        getattr(args, "choice_score_normalization", "auto"),
        "--cache-images",
        args.cache_images,
        "--data-dir",
        args.data_dir,
    ...
    add_if(command, getattr(args, "train_on_val", False), "--train-on-val")
    balance_sampler = getattr(args, "balance_sampler", "none")
    if balance_sampler != "none":
        command.extend(["--balance-sampler", balance_sampler])
...
def add_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Include grade level and subject in the question context (e.g. 'grade8 natural science').",
    )
    parser.add_argument(
        "--score-temperature",
        type=float,
        default=1.0,
        help="Temperature for log-softmax during candidate scoring (<1 sharpens, >1 flattens). Default 1.0.",
    )
    parser.add_argument(
        "--cache-images",
        choices=["none", "bytes"],
...
def add_artifact_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--run-name",
        default=None,
        help="Stable name for this run; used for outputs/<run-name> when the default output dir is unchanged and for artifacts.",
    )
    parser.add_argument(
        "--artifacts-root",
        default=DEFAULT_ARTIFACTS_ROOT,
        help=f"Root directory for organized run artifacts. Defaults to {DEFAULT_ARTIFACTS_ROOT}.",
    )
    parser.add_argument(
        "--artifact-dir",
        default=None,
        help="Exact artifact directory for this run. Overrides --artifacts-root/--run-name.",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train_parser.add_argument(
        "--extra-eval-csvs",
        default=None,
        help="Comma-separated extra labeled CSVs to evaluate after training, e.g. data/strict_split_balanced/val.csv.",
    )
    train_parser.add_argument(
        "--train-on-val",
        action="store_true",
    ...
    train_parser.add_argument(
        "--train-objective",
        choices=["generation", "choice_ranking"],
        default="generation",
        help="generation trains only the gold answer text; choice_ranking optimizes a cross-entropy over all answer choices.",
    )
    train_parser.add_argument(
        "--choice-score-normalization",
        choices=["auto", "sum", "mean"],
        default="auto",
        help="Candidate score normalization for --train-objective choice_ranking. auto follows --answer-format.",
    )
    ...
    train_parser.add_argument(
        "--lr-scheduler-type",
        default="cosine",
        help="Learning rate scheduler type (cosine, linear, constant_with_warmup, etc.).",
    )
    ...
    train_parser.add_argument("--max-grad-norm", type=float, default=1.0, help="Gradient clipping max norm.")
    train_parser.add_argument(
        "--optim",
        default="adamw_torch",
        help="Optimizer name passed to TrainingArguments (e.g. adamw_torch, adamw_torch_fused, adafactor).",
    )
    train_parser.add_argument("--logging-steps", type=int, default=10)
    train_parser.add_argument(
        "--metrics-log-path",
        default=None,
        help="CSV path for train/validation metrics; defaults to <artifact-dir>/metrics/metrics_log.csv.",
    )
    ...
    train_parser.add_argument(
        "--predict-after-train",
        action="store_true",
        help="After post-train eval, score data/test.csv and write submission plus score details into the artifact dir.",
    )
    ...
    eval_parser.add_argument("--predictions-path", default=None)
    eval_parser.add_argument(
        "--metrics-log-path",
        default=None,
        help="CSV path for eval metrics; defaults to the predictions directory.",
    )
    ...
    calibrate_parser.add_argument("--bias-step", type=float, default=0.02)
    calibrate_parser.add_argument("--max-iterations", type=int, default=5)
    ...
    add_artifact_args(predict_parser)
    predict_parser.add_argument("--checkpoint", default="outputs/smolvlm_lora/final_adapter")
    predict_parser.add_argument("--submission-path", default=None)
    ...
    ensemble_vote_parser = subparsers.add_parser(
        "ensemble-vote",
        help="Create a submission by weighted majority vote over submission CSVs.",
    )
    add_data_args(ensemble_vote_parser)
    ensemble_vote_parser.add_argument("--submission-paths", nargs="+", required=True)
    ensemble_vote_parser.add_argument("--output-path", required=True)
    ensemble_vote_parser.add_argument(
        "--weights",
        default=None,
        help="Optional comma-separated weights, one per input path. Ties use earlier paths first.",
    )

    ensemble_scores_parser = subparsers.add_parser(
        "ensemble-scores",
        help="Create a submission by averaging per-choice score-detail CSVs.",
    )
    add_data_args(ensemble_scores_parser)
    add_artifact_args(ensemble_scores_parser)
    ensemble_scores_parser.add_argument("--score-detail-paths", nargs="+", required=True)
    ensemble_scores_parser.add_argument("--output-path", default=None)
    ensemble_scores_parser.add_argument(
        "--weights",
        default=None,
        help="Optional comma-separated weights, one per input path.",
    )
    ensemble_scores_parser.add_argument(
        "--score-column",
        choices=["auto", "scores", "adjusted_scores"],
        default="auto",
        help="Use adjusted_scores when present by default, otherwise scores.",
    )
    ensemble_scores_parser.add_argument(
        "--score-normalize",
        choices=["none", "center", "zscore"],
        default="none",
        help="Optional per-row normalization before score averaging.",
    )
    ...
    autotune_parser.add_argument("--train-csv", default=None)
    autotune_parser.add_argument("--train-on-val", action="store_true")
    autotune_parser.add_argument(
        "--train-objective",
        choices=["generation", "choice_ranking"],
        default="generation",
        help="Training objective to benchmark.",
    )
    autotune_parser.add_argument(
        "--choice-score-normalization",
        choices=["auto", "sum", "mean"],
        default="auto",
        help="Candidate score normalization for choice-ranking autotune runs.",
    )
...
def main() -> None:
    ...
    if args.command == "download":
        ...
        predict(args)
    elif args.command == "ensemble-vote":
        ensemble_vote(args)
    elif args.command == "ensemble-scores":
        ensemble_scores(args)
